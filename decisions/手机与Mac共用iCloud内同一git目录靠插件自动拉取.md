...
**何时失效**:如果冲突副本频繁出现,说明两端经常同时开着 Obsidian 抢写 `.git`,这个方案就该放弃,回退到"手机独立 clone 本地 vault、iCloud vault 只留 Mac 用"。历史方案与坑见 pitfalls/git仓库放iCloud目录可能因大量小文件同步冲突、decisions/vault用xattr标记隔离git目录避免插件报错。
